...
3. Follow its documented path where survey geometry is available.
4. View a reconstruction whose controls expose the evidence and assumptions behind the scene.
5. Compare documented damage locations with the reconstruction without inventing when individual structures failed.

The archive can grow faster than the detailed reconstructions. A storm with only a location and historical account can have a useful record without receiving an invented photorealistic animation.

## Decisions still open

* Finished delivery: public browser app, downloadable Windows app, or private local app first.
* User's first three preferred reconstruction subjects.
* Services/assets budget. Current work uses free public data and existing local tools.

The common catalogue is independent of those choices. No paid service, public deployment, or public repository has been set up.
